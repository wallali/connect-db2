...
module.exports = function (session) {
    ...
    /**
     * User options used by this store.
     */
    var options = {};
    ...
    var Db2Store = function (opt, connection) {
        ...
        if (connection) {
            ...
            var dsn = options.dsn ||
                'DRIVER={DB2};DATABASE=' + options.database +
                ';UID=' + options.user +
    ...
    Db2Store.prototype.get = function (sid, fn) {
        ...
        var sql = util.format('SELECT "%s" AS "data" FROM "%s" WHERE "%s" = ?',
            options.schema.columnNames.data,
            options.schema.tableName,
            options.schema.columnNames.session_id);

        assert(store._client.connected);
    ...
    Db2Store.prototype.set = function (sid, sess, fn) {
        ...
        var expires = getExpires(sess, options.expiration);
    ...
    Db2Store.prototype.length = function (fn) {
        ...
        var sql = util.format('SELECT COUNT(*) AS "length" FROM "%s"', options.schema.tableName);
    ...
    Db2Store.prototype.clear = function (fn) {
        ...
        var sql = util.format('DELETE FROM "%s"', options.schema.tableName);
    ...
    Db2Store.prototype.hasDatabaseTable = function (fn) {
        ...
        assert(store._client.connected);
        store._client.query(sql, [options.schema.tableName], function (err, rows) {

            if (err) {
                debug('Failed to get %s table details', options.schema.tableName);
                debug(err);
                return fn(err);
            }
    ...
    Db2Store.prototype.createDatabaseTable = function (fn) {
        ...
        debug('Creating table %s', options.schema.tableName);
    ...
    Db2Store.prototype.dropDatabaseTable = function (fn) {
        ...
        debug('Dropping table %s', options.schema.tableName);

        var sql = util.format('DROP TABLE "%s"', options.schema.tableName);
